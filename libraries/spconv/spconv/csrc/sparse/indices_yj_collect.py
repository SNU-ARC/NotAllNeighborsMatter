...
class SparseConvIndicesKernel(pccm.ParameterizedClass):
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_prune_edge_0(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
        ...
        code.raw(f"""
        int filter_offset = blockIdx.y;
        int counter_filter_offset = RS - 1 - filter_offset;
        uint32_t filter_mask_out = (1u << filter_offset);
        uint32_t filter_mask_in = (1u << counter_filter_offset);

        loc_iter.set_filter_offset(filter_offset);
        int indices_pair_size_mul_RS = indices_pair_size * RS;
        
        int filter_offset_mul_indices_pair_size = filter_offset * indices_pair_size;
        int filter_offset_mul_indices_pair_size_1 = counter_filter_offset * indices_pair_size;
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_prune_edge_1(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
        ...
        code.arg("RS", "int")
        code.arg("kernel_idx_map", f"tv::array<uint32_t, 27>")

        code.raw(f"""
        int org_filter_offset = blockIdx.y;
        uint32_t check = (1u<<org_filter_offset)&0b100000001000000000100000001;
        if (check==0) {{
            int filter_offset = kernel_idx_map[org_filter_offset];
            int counter_filter_offset = kernel_idx_map[RS - 1 - org_filter_offset];
            uint32_t filter_mask_out = (1u << filter_offset);
            uint32_t filter_mask_in = (1u << counter_filter_offset);

            loc_iter.set_filter_offset(org_filter_offset);
            int indices_pair_size_mul_RS = indices_pair_size * RS;
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_prune_edge_2(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
        ...
        code.raw(f"""
        int org_filter_offset = blockIdx.y;
        uint32_t check = (1u<<org_filter_offset)&0b101000101000000000101000101;
        if (check==0) {{
            int filter_offset = kernel_idx_map[org_filter_offset];
            int counter_filter_offset = kernel_idx_map[RS - 1 - org_filter_offset];
            uint32_t filter_mask_out = (1u << filter_offset);
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_prune_edge_3(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
        ...
        code.raw(f"""
        int org_filter_offset = blockIdx.y;
        uint32_t check = (1u<<org_filter_offset)&0b101000101101000101101000101;
        if (check==0) {{
            int filter_offset = kernel_idx_map[org_filter_offset];
            int counter_filter_offset = kernel_idx_map[RS - 1 - org_filter_offset];
            uint32_t filter_mask_out = (1u << filter_offset);
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_prune_edge_4(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
        ...
        code.raw(f"""
        int org_filter_offset = blockIdx.y;
        uint32_t check = (1u<<org_filter_offset)&0b101101101101000101101101101;
        if (check==0) {{
            int filter_offset = kernel_idx_map[org_filter_offset];
            int counter_filter_offset = kernel_idx_map[RS - 1 - org_filter_offset];
            uint32_t filter_mask_out = (1u << filter_offset);
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_prune_edge_5(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
        ...
        code.raw(f"""
        int org_filter_offset = blockIdx.y;
        uint32_t check = (1u<<org_filter_offset)&0b111101111101000101111101111;
        if (check==0) {{
            int filter_offset = kernel_idx_map[org_filter_offset];
            int counter_filter_offset = kernel_idx_map[RS - 1 - org_filter_offset];
            uint32_t filter_mask_out = (1u << filter_offset);
    ...
    #     code.raw(f"""
    #     int filter_offset = blockIdx.y;
    #     uint32_t check = (1u<<filter_offset)&0b100000001000000000100000001;
    #     if (check==0) {{
    #         int counter_filter_offset = RS - 1 - filter_offset;
    #         uint32_t filter_mask_out = (1u << filter_offset);
    #         uint32_t filter_mask_in = (1u << counter_filter_offset);
    ...
    #     code.raw(f"""
    #     int filter_offset = blockIdx.y;
    #     uint32_t check = (1u<<filter_offset)&0b101000101000000000101000101;
    #     if (check==0) {{
    #         int counter_filter_offset = RS - 1 - filter_offset;
    #         uint32_t filter_mask_out = (1u << filter_offset);
    #         uint32_t filter_mask_in = (1u << counter_filter_offset);
    ...
    #     code.raw(f"""
    #     int filter_offset = blockIdx.y;
    #     uint32_t check = (1u<<filter_offset)&0b101000101101000101101000101;
    #     if (check==0) {{
    #         int counter_filter_offset = RS - 1 - filter_offset;
    #         uint32_t filter_mask_out = (1u << filter_offset);
    #         uint32_t filter_mask_in = (1u << counter_filter_offset);
    ...
    #     code.raw(f"""
    #     int filter_offset = blockIdx.y;
    #     uint32_t check = (1u<<filter_offset)&0b101101101101000101101101101;
    #     if (check==0) {{
    #         int counter_filter_offset = RS - 1 - filter_offset;
    #         uint32_t filter_mask_out = (1u << filter_offset);
    #         uint32_t filter_mask_in = (1u << counter_filter_offset);
    ...
    #     code.raw(f"""
    #     int filter_offset = blockIdx.y;
    #     uint32_t check = (1u<<filter_offset)&0b111101111101000101111101111;
    #     if (check==0) {{
    #         int counter_filter_offset = RS - 1 - filter_offset;
    #         uint32_t filter_mask_out = (1u << filter_offset);
    #         uint32_t filter_mask_in = (1u << counter_filter_offset);
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_yj_prune_edge_0(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
        ...
        code.arg("RS", "int")
        # YJ Parameter
        code.arg("yj_feature_mask", f"const uint8_t*")
        ...
        if (filter_offset == (RS / 2)){{
            ...
            for (int output_index : tv::KernelLoopX<int>(num_indices)) {{
                if(yj_feature_mask[output_index]==1) {{
                    // find input offset from output offset
                    tv::array<int, {self.ndim + 1}> nhw_offset;
                    // table: input indice coord to output index (or output indice coord to input index)
                    if (loc_iter.query_nhw(indices_in + output_index * {self.ndim + 1}, nhw_offset)){{
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_yj_prune_edge_1(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_yj_prune_edge_2(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_yj_prune_edge_3(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_yj_prune_edge_4(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_yj_prune_edge_5(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
    ...
    def generate_conv_inds_stage2(self):
        ...
        hash.clear(custream);
        // std::cout << "Lanucher build hash" << std::endl;
        ...
        // std::cout << "out inds: " << out_inds << std::endl;
    ...
    def generate_subm_conv_inds(self):
        code = pccm.FunctionCode()
        code.arg("indices, hashdata", "tv::Tensor")
        code.arg("indice_pairs, out_inds, indice_num_per_loc, yj_feature_mask", "tv::Tensor")
            ...
            if (indice_pair_mask.dim(0) == 2){{
                ...
                if(!yj_feature_mask.empty()) {{
                    ...
                    if (prune_edge == 0) {{     
                        ...
                        launcher_num_act_in(calc_subm_conv_indices_mask_yj_prune_edge_0<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, yj_feature_mask.data_ptr<uint8_t>());
                    }}
                    // [YJ] prune 4 edges
                    else if (prune_edge == 1) {{
                        // tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        // lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << (kv/2)), indice_pair_mask.size());

                        // YJ, map for kernel index
                        TV_ASSERT_RT_ERR(kv==27, "[YJ] kv must be 27 for now..");
                        tv::array<uint32_t, 27> kernel_idx_map;
                        kernel_idx_map[0]  = 23;
                        kernel_idx_map[1]  = 0;
                        kernel_idx_map[2]  = 1;
                        kernel_idx_map[3]  = 2;
                        ...
                        kernel_idx_map[10] = 8;
                        kernel_idx_map[11] = 9;
                        kernel_idx_map[12] = 10;
                        kernel_idx_map[13] = 11;
                        kernel_idx_map[14] = 12;
                        kernel_idx_map[15] = 13;
                        kernel_idx_map[16] = 14;
                        kernel_idx_map[17] = 15;
                        kernel_idx_map[18] = 25;
                        kernel_idx_map[19] = 16;
                        kernel_idx_map[20] = 17;
                        kernel_idx_map[21] = 18;
                        kernel_idx_map[22] = 19;
                        kernel_idx_map[23] = 20;
                        kernel_idx_map[24] = 21;
                        kernel_idx_map[25] = 22;
                        kernel_idx_map[26] = 26;
                        tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << 11), indice_pair_mask.size());

                        launcher_num_act_in(calc_subm_conv_indices_mask_yj_prune_edge_1<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, yj_feature_mask.data_ptr<uint8_t>(), kernel_idx_map);
                    }}
                    // [YJ] prune 8 edges
                    else if (prune_edge == 2) {{
                        // tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        // lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << (kv/2)), indice_pair_mask.size());
                        ...
                        kernel_idx_map[12] = 8;
                        kernel_idx_map[13] = 9;
                        kernel_idx_map[14] = 10;
                        kernel_idx_map[15] = 11;
                        kernel_idx_map[16] = 12;
                        kernel_idx_map[17] = 13;
                        kernel_idx_map[18] = 23;
                        kernel_idx_map[19] = 14;
                        kernel_idx_map[20] = 24;
                        kernel_idx_map[21] = 15;
                        kernel_idx_map[22] = 16;
                        kernel_idx_map[23] = 17;
                        kernel_idx_map[24] = 25;
                        kernel_idx_map[25] = 18;
                        kernel_idx_map[26] = 26;

                        tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << 9), indice_pair_mask.size());

                        launcher_num_act_in(calc_subm_conv_indices_mask_yj_prune_edge_2<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, yj_feature_mask.data_ptr<uint8_t>(), kernel_idx_map);
                    }}
                    // [YJ] prune 12 edges
                    else if (prune_edge == 3) {{
                        // tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        // lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << (kv/2)), indice_pair_mask.size());
                        ...
                        kernel_idx_map[9]  = 19;
                        kernel_idx_map[10] = 5;
                        kernel_idx_map[11] = 20;
                        kernel_idx_map[12] = 6;
                        kernel_idx_map[13] = 7;
                        kernel_idx_map[14] = 8;
                        kernel_idx_map[15] = 21;
                        kernel_idx_map[16] = 9;
                        kernel_idx_map[17] = 22;
                        kernel_idx_map[18] = 23;
                        kernel_idx_map[19] = 10;
                        kernel_idx_map[20] = 24;
                        kernel_idx_map[21] = 11;
                        kernel_idx_map[22] = 12;
                        kernel_idx_map[23] = 13;
                        kernel_idx_map[24] = 25;
                        kernel_idx_map[25] = 14;
                        kernel_idx_map[26] = 26;

                        tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << 7), indice_pair_mask.size());

                        launcher_num_act_in(calc_subm_conv_indices_mask_yj_prune_edge_3<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, yj_feature_mask.data_ptr<uint8_t>(), kernel_idx_map);
                    }}
                    // [YJ] prune 16 edges
                    else if (prune_edge == 4) {{
                        // tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        // lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << (kv/2)), indice_pair_mask.size());
                        ...
                        kernel_idx_map[9]  = 17;
                        kernel_idx_map[10] = 3;
                        kernel_idx_map[11] = 18;
                        kernel_idx_map[12] = 4;
                        kernel_idx_map[13] = 5;
                        kernel_idx_map[14] = 6;
                        kernel_idx_map[15] = 19;
                        kernel_idx_map[16] = 7;
                        kernel_idx_map[17] = 20;
                        kernel_idx_map[18] = 21;
                        kernel_idx_map[19] = 8;
                        kernel_idx_map[20] = 22;
                        kernel_idx_map[21] = 23;
                        kernel_idx_map[22] = 9;
                        kernel_idx_map[23] = 24;
                        kernel_idx_map[24] = 25;
                        kernel_idx_map[25] = 10;
                        kernel_idx_map[26] = 26;

                        tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << 5), indice_pair_mask.size());

                        launcher_num_act_in(calc_subm_conv_indices_mask_yj_prune_edge_4<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, yj_feature_mask.data_ptr<uint8_t>(), kernel_idx_map);
                    }}
                    // [YJ] prune 20 edges
                    else if (prune_edge ==5) {{
                        // tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        // lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << (kv/2)), indice_pair_mask.size());
                        ...
                        kernel_idx_map[9]  = 15;
                        kernel_idx_map[10] = 1;
                        kernel_idx_map[11] = 16;
                        kernel_idx_map[12] = 2;
                        kernel_idx_map[13] = 3;
                        kernel_idx_map[14] = 4;
                        kernel_idx_map[15] = 17;
                        kernel_idx_map[16] = 5;
                        kernel_idx_map[17] = 18;
                        kernel_idx_map[18] = 19;
                        kernel_idx_map[19] = 20;
                        kernel_idx_map[20] = 21;
                        kernel_idx_map[21] = 22;
                        kernel_idx_map[22] = 6;
                        kernel_idx_map[23] = 23;
                        kernel_idx_map[24] = 24;
                        kernel_idx_map[25] = 25;
                        kernel_idx_map[26] = 26;

                        tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << 3), indice_pair_mask.size());
                        launcher_num_act_in(calc_subm_conv_indices_mask_yj_prune_edge_5<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, yj_feature_mask.data_ptr<uint8_t>(), kernel_idx_map);
                    }}
                    else {{
                        TV_ASSERT_RT_ERR(false, "Wrong prune_edge value");
                    }}
                }}
                else {{
                    // [YJ] original
                    if (prune_edge == 0) {{
                        tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << (kv/2)), indice_pair_mask.size());
                        launcher_num_act_in(calc_subm_conv_indices_mask_prune_edge_0<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                    ...
                    // [YJ] prune 4 edges
                        ...
                        launcher_num_act_in(calc_subm_conv_indices_mask_prune_edge_1<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, kernel_idx_map);
                    }}
                    // [YJ] prune 8 edges
                    else if (prune_edge == 2) {{
                        // tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        ...
                        launcher_num_act_in(calc_subm_conv_indices_mask_prune_edge_2<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, kernel_idx_map);
                    }}
                    // [YJ] prune 12 edges
                        ...
                        launcher_num_act_in(calc_subm_conv_indices_mask_prune_edge_3<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, kernel_idx_map);
                    }}
                    // [YJ] prune 20 edges
                        ...
                        tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << 5), indice_pair_mask.size());
                        launcher_num_act_in(calc_subm_conv_indices_mask_prune_edge_4<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, kernel_idx_map);
                    }}
                    // [YJ] prune 20 edges
                        ...
                        tv::cuda::Launch lanucher_fill(indice_pair_mask.size(), custream);
                        lanucher_fill(cudakers::fill_kernel<uint32_t>, indice_pair_mask.data_ptr<uint32_t>(), (1 << 3), indice_pair_mask.size());
                        launcher_num_act_in(calc_subm_conv_indices_mask_prune_edge_5<table_t>, loc_iter, hash, 
                            indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                            indice_pair_mask.data_ptr<uint32_t>(), indices.dim(0), indice_pairs.dim(2), kv, kernel_idx_map);
                    }}
                    else {{
