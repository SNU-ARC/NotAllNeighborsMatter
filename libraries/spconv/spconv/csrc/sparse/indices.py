...
class CudaCommonKernel(pccm.ParameterizedClass):
    ...
    @pccm.cuda.cuda_global_function
    def yj_bit_count_function(self):
        code = pccm.FunctionCode()
        code.targ("T")
        code.arg("pair_mask_clone", f"const T*")
        code.arg("temp", f"T*")
        code.arg("num_feat", f"int")
        code.arg("kv", f"int")

        code.raw(f"""
        int x = blockIdx.x*blockDim.x + threadIdx.x;    // num_feat
        int y = blockIdx.y*blockDim.y + threadIdx.y;    // kv
        auto pointerpointer = temp + x*kv;
        pointerpointer[y] = (pair_mask_clone[x] >> y)&1;
        """)
        return code

    @pccm.cuda.cuda_global_function
    def unset_pair_yj(self):
        code = pccm.FunctionCode()
        code.targ("T")
        code.arg("pair_fwd", f"T*")
        code.arg("prune_edge, kv, num_feat", f"int")

        code.raw(f"""
        // int x = blockIdx.x*blockDim.x + threadIdx.x;    // num_feat
        // int y = blockIdx.y*blockDim.y + threadIdx.y;    // kv
        // auto check = (1u<<y)&prune_edge;
        // if (check == 0) {{
        //     auto pointerpointer = pair_fwd + num_feat * y;
        //     pointerpointer[x] = -1;
        // }}

        // int x = blockIdx.x*blockDim.x + threadIdx.x;    // num_feat
        int x = threadIdx.x;    // num_feat
        int y = blockIdx.y;    // kv
        if (y<kv) {{
            auto check = (1u<<y)&prune_edge;
            if (check == 0) {{
...
class SparseConvIndicesKernel(pccm.ParameterizedClass):
    ...
    # @pccm.cuda.cuda_global_function
    # def calc_conv_indices_stage1_mask_edge_1(self):
    #     code = pccm.FunctionCode()
    #     code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
    ...
    # @pccm.cuda.cuda_global_function
    # def calc_conv_indices_stage1_mask_edge_2(self):
    #     code = pccm.FunctionCode()
    #     code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
    ...
    # @pccm.cuda.cuda_global_function
    # def calc_conv_indices_stage1_mask_edge_3(self):
    #     code = pccm.FunctionCode()
    #     code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
    ...
    # @pccm.cuda.cuda_global_function
    # def calc_conv_indices_stage1_mask_edge_4(self):
    #     code = pccm.FunctionCode()
    #     code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
    ...
    # @pccm.cuda.cuda_global_function
    # def calc_conv_indices_stage1_mask_edge_5(self):
    #     code = pccm.FunctionCode()
    #     code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
    ...
    @pccm.cuda.cuda_global_function
    def calc_conv_indices_stage2_mask_bit_count(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("table", f"TTable")  # [N, ndim + 1]
        code.arg("indice_pairs_fwd",
        ...
        code.arg("yj_bit_count_fwd", f"int32_t*")
        ...
        for (int input_index : tv::KernelLoopX<int>(num_indices_in)) {{
            ...
            if (output_coord_offset > -1){{
                ...
                if (ptr){{
                    ...
                    auto pointerpointer = yj_bit_count_fwd + gridDim.y * output_index;
                    pointerpointer[filter_offset] = 1;
                }}
            }}
        }}
        """)
    ...
    @pccm.cuda.cuda_global_function
    def calc_conv_indices_stage2_mask_output_bit_count(self):
        code = pccm.FunctionCode()
        code.arg("indice_pairs_bwd",
                 f"int*")  # [kernelProd, MaxSize], out -> inp
        code.arg("mask_bwd", f"uint32_t*")  # [kernelProd]
        code.arg("yj_bit_count_bwd", f"int32_t*")
        ...
        for (int input_index : tv::KernelLoopX<int>(num_indices_in)) {{
            uint32_t mask = 0;
            auto pointerpointer = yj_bit_count_bwd + kv*input_index;
    ...
    @pccm.cuda.cuda_global_function
    def calc_subm_conv_indices_mask_bit_count(self):
        code = pccm.FunctionCode()
        code.targ("TTable")
        code.arg("loc_iter", f"ConvLocIter")  # [N, ndim + 1]
        code.arg("table", f"TTable")  # [N, ndim + 1]
        ...
        code.arg("indice_pairs",
                 f"{self.dtype_indices}*")  # [2, kernelProd, MaxSize]
        code.arg("mask", f"uint32_t*")  # [kernelProd]
        code.arg("yj_bit_count", f"int32_t*")
        ...
        if (filter_offset == (RS / 2)){{
            for (int i : tv::KernelLoopX<int>(num_indices)) {{
                ...
                indice_pairs[filter_offset_mul_indices_pair_size + i] = i;
                indice_pairs[indices_pair_size_mul_RS + filter_offset_mul_indices_pair_size + i] = i;
                auto pointerpointer = yj_bit_count + RS*i;
                pointerpointer[filter_offset] = 1;
            }}
        }} else {{
            for (int output_index : tv::KernelLoopX<int>(num_indices)) {{
                ...
                if (loc_iter.query_nhw(indices_in + output_index * {self.ndim + 1}, nhw_offset)){{
                    ...
                    if (!item.empty()) {{
                        ...
                        auto pointerpointer = yj_bit_count + RS*output_index;
                        pointerpointer[filter_offset]=1;
                        auto pointerpointer2 = yj_bit_count + RS*input_index;
                        pointerpointer2[RS - 1 - filter_offset]=1;
                    }}
                }}
            }}
        }}
    ...
    def generate_conv_inds_stage2_mask(self):
        ...
        yj_bit_count_fwd.zero_(ctx);
        yj_bit_count_bwd.zero_(ctx);
        ...
        if (!mask_bwd.empty()){{
            ...
            if(yj_bit_count_fwd.empty()) {{
                ...
                launcher_num_act_in(calc_conv_indices_stage2_mask_bit_count<table_t>, hash, 
                    indice_pairs_fwd.data_ptr<int>(), indice_pairs_bwd.data_ptr<int>(), 
                    mask_fwd.data_ptr<uint32_t>(), yj_bit_count_fwd.data_ptr<int>(),
                    indice_pairs_bwd.dim(1), indice_pairs_fwd.dim(1));
            }}
            // tv::ssprint("calc_conv_indices_stage2_mask", timer.report() / 1000.0);
            if(yj_bit_count_bwd.empty()) {{
                ...
                launcher_num_act_in_no_y(calc_conv_indices_stage2_mask_output_bit_count, indice_pairs_bwd.data_ptr<int>(), 
                    mask_bwd.data_ptr<uint32_t>(), yj_bit_count_bwd.data_ptr<int>(),
                    indice_pairs_bwd.dim(1), kv);
            }}
    ...
    def generate_subm_conv_inds(self):
        ...
        if (!yj_bit_count.empty()) {{
            yj_bit_count.zero_(ctx);
        }}
        // YJ
        ...
        if (!indice_pair_mask.empty()){{
            ...
            if (indice_pair_mask.dim(0) == 2){{
                ...
                tv::cuda::Launch lanucher_fill_indices(indice_pairs.size(), custream);
                lanucher_fill_indices(cudakers::fill_kernel<int32_t>, indice_pairs.data_ptr<int32_t>(), -1, indice_pairs.size());
                ...
                if(yj_bit_count.empty()) {{
                    ...
                    launcher_num_act_in(calc_subm_conv_indices_mask_bit_count<table_t>, loc_iter, hash, 
                        indices.data_ptr<int>(), indice_pairs.data_ptr<int>(), 
                        indice_pair_mask.data_ptr<uint32_t>(), yj_bit_count.data_ptr<int>(), indices.dim(0), indice_pairs.dim(2), kv);
                }}
            }}
        }}else{{
