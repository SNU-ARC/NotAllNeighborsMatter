...
def get_indice_pairs_implicit_gemm(
    ...
    if subm:
        ...
        #                   dtype=indices.dtype,
        #                   device=indices.device)
        pair = torch.empty((2, kv, indices.shape[0]),
                          dtype=indices.dtype,
                          device=indices.device)
    else:
    ...
    #                                  dtype=indices.dtype,
    #                                  device=indices.device)
    indice_num_per_loc = torch.empty((kv, ),
                                     dtype=indices.dtype,
                                     device=indices.device)
    ...
    if subm:
        ...
        if bitcount_export==True:
            # pair_mask_clone_tv = torch_tensor_to_tv(pair_mask[0])
            yj_bit_count = torch.empty((indices.shape[0], kv), dtype=indices.dtype, device=pair_mask.device)

            # SpconvOps.generate_bitcount(pair_mask_clone_tv, yj_bit_count_tv, stream_int=stream)
        else:
            yj_bit_count = torch.Tensor()

        yj_bit_count_tv = torch_tensor_to_tv(yj_bit_count)

        with timer.record("gen_subm_inds", stream):
            SpconvOps.generate_subm_conv_inds(inds_tv,          # [YJ] Not Changed
                                              hashdata_tv,      # [YJ] Changed (Not used)
                                              pair_tv,          # [YJ] Changed
                                              out_inds_tv,      # [YJ] Not Changed
                                              indice_num_per_loc_tv,
                                              batch_size=batch_size,
                                              input_dims=spatial_shape,
                                              ksize=ksize,
                                              dilation=dilation,
                                              indice_pair_mask=pair_mask_tv,    # [YJ] Changed
                                              yj_bit_count=yj_bit_count_tv,
                                              stream_int=stream)
        ...
        if bitcount_export==True:
            yj_bit_count = torch.index_select(yj_bit_count, 0, mask_argsort[0])
        ...
        return (out_inds, pair[0], pair[1],
                pair_mask_in_splits, [], mask_argsort_in_splits, [], masks, yj_bit_count, None)
        ...
        indice_pairs_uniq = torch.empty((pair.numel() + 1, ),    # [YJ] pair.shape = [kv, # features], pair.numel()==kv * #features
                                        dtype=indices.dtype,
                                        device=indices.device)
        indice_pairs_uniq_tv = torch_tensor_to_tv(indice_pairs_uniq)

        with timer.record("gen_conv_inds_stage1", stream):
            SpconvOps.generate_conv_inds_mask_stage1(inds_tv,               # [YJ] Not Changed
                                                     pair_bwd_tv,           # [YJ] Changed
                                                     indice_pairs_uniq_tv,  # [YJ] Changed
                                                     indice_num_per_loc_tv, # [YJ] Changed
        ...
        #                             dtype=torch.int32,
        #                             device=indices.device)
        pair_mask_fwd = torch.empty((mask_split_count, num_act_out),
                                    dtype=torch.int32,
                                    device=indices.device)
        ...
        if is_train:
            ...
            #                             dtype=torch.int32,
            #                             device=indices.device)
            pair_mask_bwd = torch.empty((mask_split_count, indices.shape[0]),
                                        dtype=torch.int32,
                                        device=indices.device)
        ...
        if bitcount_export==True:
            yj_bit_count_fwd = torch.empty((num_act_out, kv), dtype=indices.dtype, device=pair_mask_fwd.device)
            yj_bit_count_bwd = torch.empty((indices.shape[0], kv), dtype=indices.dtype, device=pair_mask_fwd.device)
        else:
            yj_bit_count_fwd = torch.Tensor()
            yj_bit_count_bwd = torch.Tensor()


        yj_bit_count_fwd_tv = torch_tensor_to_tv(yj_bit_count_fwd)
        yj_bit_count_bwd_tv = torch_tensor_to_tv(yj_bit_count_bwd)

        with timer.record("gen_conv_inds_stage2", stream):
            SpconvOps.generate_conv_inds_mask_stage2(inds_tv,           # [YJ] Not Changed
                                                     hashdata_tv,       # [YJ] Changed
                                                     pair_fwd_tv,       # [YJ] Changed
                                                     pair_bwd_tv,       # [YJ] Changed
                                                     uniq_res_tv,       # [YJ] Not Changed
                                                     out_inds_tv,       # [YJ] Changed
                                                     pair_mask_fwd_tv,  # [YJ] Changed
                                                     pair_mask_bwd_tv,  # [YJ] Changed
                                                     yj_bit_count_fwd_tv,
                                                     yj_bit_count_bwd_tv,
                                                     num_out_act=num_act_out,
                                                     batch_size=batch_size,
        ...
        if bitcount_export==True:
            yj_bit_count_fwd = torch.index_select(yj_bit_count_fwd, 0, mask_argsort_fwd[0])
            yj_bit_count_bwd = torch.index_select(yj_bit_count_bwd, 0, mask_argsort_bwd[0])
        ...
        return (out_inds, pair_fwd, pair_bwd,
                pair_mask_fwd_splits, pair_mask_bwd_splits,
                mask_argsort_fwd_splits, mask_argsort_bwd_splits, masks, yj_bit_count_fwd, yj_bit_count_bwd)
...
def implicit_gemm(features: torch.Tensor,
                  ...
                  selected_tune_res: str = None,
                  yj_bit_count: torch.Tensor = None,
                  bitcount_export: bool = False,
                  yj_bitcount_export = None,
                  prune_edge=None):
    stream = get_current_stream()
    # if DEBUG:
    ...
    pair_fwd_tv = torch_tensor_to_tv(pair_fwd)
    # SpconvOps.unset_pair(pair_fwd_tv, prune_edge, stream)
    # print(pair_fwd.shape)
    # pair_fwd_tv = torch_tensor_to_tv(pair_fwd)
    ...
    if bitcount_export:
        num_chunk = int((yj_bit_count.shape[0]+mask_width-1)/mask_width)
        yj_bit_count = F.pad(yj_bit_count, pad=(0, 0, 0, num_chunk*mask_width-yj_bit_count.shape[0]), mode="constant", value=0)
        yj_bit_count = yj_bit_count.reshape((num_chunk, mask_width, yj_bit_count.shape[1]))
        yj_bit_count = torch.sum(yj_bit_count, dim=1, dtype=torch.int32)

        if bitcount_export is True:
            yj_bitcount_export.append({'bitcount':yj_bit_count.to(torch.float), 'mask_width': mask_width})
...
def implicit_gemm_backward(features: torch.Tensor,
    ...
    if dgrad_tune_res is None:
        ...
        dgrad_tune_res, _ = CONV.tune_and_cache(ConvOpType.kBackwardInput,
                                                ...
                                                stream=stream,
                                                fp32_accum=fp32_accum,
                                                selected_tune_res={})   # Dummy tune_res
    if wgrad_tune_res is None:
        wgrad_tune_res, _ = CONV.tune_and_cache(
            ...
            reverse_mask=False,
            mask_filter=masks[0].item(),
            mask_output=mask_output_fwd_tv[0],      # kmask for num_chunk
    ...
    with timer.record("implicit_gemm_backward", stream):
        for j in range(num_split):
            ...
            # if yj_bitcount_bwd != None:
            #     yj_bitcount_bwd_trans = yj_bitcount_bwd.transpose(-2, -1)   # kv * # output points
            #     # stat
            #     # pruned_ = (yj_bitcount_bwd_trans[2] == True).nonzero(as_tuple=True)[0]
            #     # true_pruned_indices = pair_fwd[2][yj_bitcount_bwd_trans[2]==True]
            #     # for i in range(true_pruned_indices.shape[0]):
            #     #     assert(true_pruned_indices[i] == pair_fwd[2][pruned_[i]])
            #     # print("assert passed")

            #     # bitmask=2**(yj_bitcount_bwd_trans.shape[0])-1
            #     for i in range(yj_bitcount_bwd_trans.shape[0]):
            #         pruned_indices = pair_fwd[i][yj_bitcount_bwd_trans[i]==True]
            #         pair_bwd[i,pruned_indices.long()] = -1
            #         # pair_mask_fwd_splits[j][pruned_indices.long()] &= (bitmask-(2**i))

            #     # for i in range(pruned_.shape[0]):
            #     #     assert(pair_bwd[2][pair_fwd[2][pruned_[i]]]==-1)
            #     # print("assert passed2")
