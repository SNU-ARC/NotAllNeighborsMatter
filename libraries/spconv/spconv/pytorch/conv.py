...
class SparseConvolution(SparseModule):
    ...
    def __init__(self,
        ...
        self.selected_tune_res = selected_tune_res

        self.bitcount_export = bitcount_export
        self.yj_bitcount_export = list()
    ...
    def reset_parameters(self):
        # print("[YJ] Default parameter setting")
        ...
        # print("[YJ] TorchSparse parameter setting")
        ...
        # kernel_volume = int(np.prod(make_ntuple(self.kernel_size, ndim=3)))
    ...
    def forward(self, input: SparseConvTensor):
        ...
        if self.conv1x1:
            if FILTER_HWIO:
                ...
                # features = torch.empty([input.features.shape[0], self.out_channels], device=device)
            if self.bias is not None:
                features += self.bias
        ...
        with profile_ctx:
            if algo == ConvAlgo.Native:
                ...
                if self.inverse:
                    ...
                    masks = datas.masks
                    out_spatial_shape = datas.spatial_shape
                    yj_bit_count_fwd = datas.yj_bit_count_bwd
                    assert datas.ksize == self.kernel_size, "inverse conv must have same kernel size as its couple conv"
                else:
                    if self.indice_key is not None and datas is not None:
                        outids = datas.out_indices
                        ...
                        masks = datas.masks
                        yj_bit_count_fwd = datas.yj_bit_count_fwd
                        ...
                        with input._timer.namespace("gen_pairs"):
                            ...
                            try:
                                res = ops.get_indice_pairs_implicit_gemm(
                                    ...
                                    timer=input._timer,
                                    bitcount_export=self.bitcount_export)
                        ...
                        pair_fwd = res[1]
                        pair_bwd = res[2]
                        pair_mask_fwd_splits = res[3]
                        pair_mask_bwd_splits = res[4]
                        mask_argsort_fwd_splits = res[5]
                        mask_argsort_bwd_splits = res[6]
                        masks = res[7]
                        yj_bit_count_fwd = res[8]
                        yj_bit_count_bwd = res[9]
                        ...
                        if self.indice_key is not None:
                            indice_data = ImplicitGemmIndiceData(
                                ...
                                padding=self.padding,
                                dilation=self.dilation,
                                yj_bit_count_fwd=yj_bit_count_fwd,
                                yj_bit_count_bwd=yj_bit_count_bwd)
                            msg = f"your indice key {self.indice_key} already exists in this sparse tensor."
                            assert self.indice_key not in indice_dict, msg
                            indice_dict[self.indice_key] = indice_data
                if input.benchmark:
                ...
                temp_fwd = pair_mask_fwd_splits
                temp_bwd = pair_mask_bwd_splits
                if self.prune_edge > 0:
                    temp_fwd[0] &= self.prune_edge
                    if self.training and self.subm==False:
                        temp_bwd[0] &= self.prune_edge
                ...
                out_features = Fsp.implicit_gemm(
                    ...
                    input._timer, self.fp32_accum, self.inverse, self.selected_tune_res,
                    yj_bit_count_fwd,
                    self.bitcount_export,
                    self.prune_edge)
                if self.bitcount_export:
                    self.yj_bitcount_export += Fsp.SparseImplicitGemmFunction.get_yj_bitcount_export()
