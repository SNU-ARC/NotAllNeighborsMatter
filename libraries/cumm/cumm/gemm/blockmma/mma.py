...
class Mma(pccm.ParameterizedClass):
    ...
    def call_mask_sparse_k_first(self):
        ...
        // tv::printf2_once("[YJ] call_mask_sparse_k_first, RS: ", RS);
        WarpMma warp_mma;
        ...
        // int iter_count = 0;
        // tv::printf2_once("[YJ] mma iter start");
        while (!mask_iter.end){{
            // tv::printf2_once("[YJ] mask_iter: ", iter_count++);
            // tv::printf2_once("[YJ] mask_filter.filter_idx: ", mask_iter.filter_idx);
            // tv::printf2_once("[YJ] gemm_k_iterations: ", gemm_k_iterations);
            // tv::printf2_once("[YJ] num_warp_mma_iters: ", {self.spec.num_warp_mma_iters});
            for (int i = 0; i < gemm_k_iterations; ++i){{
                ...
                for (int warp_mma_k = 0; warp_mma_k < {self.spec.num_warp_mma_iters}; ++warp_mma_k){{
                    ...
                    ++warp_iter_A;
                    // tv::printf2_once("[YJ] iterA++");
                    ++warp_iter_B;
                    // tv::printf2_once("[YJ] iterB++");
    ...
    def call_mask_sparse_wgrad(self):
        ...
        code.raw(f"""
        // tv::printf2_once("[YJ] call_mask_sparse_wgrad");
    ...
    def call_mask_sparse_k_first_ffs(self):
        ...
        code.arg("RS", f"int")
        code.raw(f"""
        // tv::printf2_once("[YJ] call_mask_sparse_k_first_ffs");
        accumulators = src_accumulators;
        {self.input_spec.input_iter_a.fragment_t} input_frag_A;
        {self.input_spec.input_iter_b.fragment_t} input_frag_B;
        """)
    ...
    def call_mask_sparse_filter_first(self):
        ...
        code.arg("RS", f"int")
        code.raw(f"""
        // tv::printf2_once("[YJ] call_mask_sparse_filter_first");
        accumulators = src_accumulators;
        {self.input_spec.input_iter_a.fragment_t} input_frag_A;
        {self.input_spec.input_iter_b.fragment_t} input_frag_B;
        """)
