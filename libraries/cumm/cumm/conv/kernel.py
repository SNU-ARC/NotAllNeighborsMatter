...
class ConvParams(pccm.ParameterizedClass):
    def __init__(self,
        ...
        self.add_member("is_train", f"bool")
    ...
    def ctor(self):
        ...
        code.ctor_init("is_train", "is_train")
...
class ConvKernel(pccm.ParameterizedClass):
    ...
    def conv_kernel(self):
        ...
        with code.macro_if_(f"defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= {arch_num})"):
            ...
            code.raw(f"""
            // tv::printf2_once("[YJ] conv kernel");
            ...
            // tv::printf2_once("[YJ] tile_offset_m: ", tile_offset_m);
            // tv::printf2_once("[YJ] tile_offset_n: ", tile_offset_n);
            // tv::printf2_once("[YJ] tile_offset_k: ", tile_offset_k);
            // tv::printf2_once("[YJ] tile_shape[2]: ", {self.tile_shape[2]});
            // tv::printf2_once("[YJ] tile_shape[0]: ", {self.tile_shape[0]});
            // tv::printf2_once("[YJ] tile_shape[1]: ", {self.tile_shape[1]});
            """)
            ...
            """)
            code.raw(f"""
            // tv::printf2_once("[YJ] a_offset: ", {a_offset});
            // tv::printf2_once("[YJ] b_offset: ", {b_offset});
            ...
            if self.mask_sparse:
                if not self.problem.op_type == ConvOpType.kBackwardWeight:
                    ...
                    if self.problem.op_type == ConvOpType.kForward:
                        ...
                        code.raw(f"""
                        if (params.is_train && params.mask_out_ptr != nullptr){{
            ...
            // tv::printf2_once("[YJ] m: ", params.m, " / n: ", params.n, " / k: ", params.k);
