...
def main():
  config = get_config()
  if config.resume and config.retrain==False:
  ...
  #   converted_checkpoint_path = config.weights.replace('.pt', '_'+'_'.join(['prune_edge'+str(config.prune_info[str(i)]['prune_edge']) for i in range(5)])+'.pt')
  #   model.convert_checkpoint(config.prune_info, config.weights, converted_checkpoint_path)

  #   logging.info('===> Loading weights: ' + converted_checkpoint_path)
  #   new_state = torch.load(converted_checkpoint_path)
  #   model.load_state_dict(new_state['state_dict'])
  ...
  # Load weights if specified by the parameter.
    ...
    # state = torch.load("/arc-share/pc_yejin/checkpoints/SpatioTemporalSegmentation/" + config.weights)
    state = torch.load(config.weights)

    # If retrain, prune edges in the weights according to the given prune_info.
    # if config.retrain:
    #     state = model.prune_checkpoint(config.prune_info, state)
      ...
      if config.lenient_weight_loading:
        ...
        if config.backend == 'torchsparse':
          ...
          for k,v in state['state_dict'].items():
            ...
            # if 'weight' in k and 'bn' not in k and len(v.shape)==5:
            #   a,b,c,d,e = v.shape
            #   k = k.replace('weight', 'kernel')
            #   if 'downsample' in k or 'final' in k:
            #     assert(b*c*d==1)
            #     v = v.reshape((e, a))
            #   else:
            #     v = v.reshape((b*c*d, e, a))
            # elif len(v.shape)==3:
            #   a, b, c = v.shape
            #   assert(a==1)
            #   v = v.reshape((b, c))
            # elif 'bn' in k or 'block' in k:
            #   if 'weight' in k:
            #     k = k.replace('weight', 'bn.weight')
            #   elif 'bias' in k:
            #     k = k.replace('bias', 'bn.bias')
            #   elif 'running_mean' in k:
            #     k = k.replace('running_mean', 'bn.running_mean')
            #   elif 'running_var' in k:
            #     k = k.replace('running_var', 'bn.running_var')
            #   elif 'num_batches_tracked' in k:
            #     continue
            # elif 'final.bias' in k:
  ...
  if config.is_train:
    ...
    test(model, train_data_loader, val_data_loader, config)
