...
STANFORD_3D_IN_PATH = dataset_dir+'s3dis/Stanford3dDataset_v1.2/'
STANFORD_3D_OUT_PATH = dataset_dir+'s3dis/preprocessing/'
...
class Stanford3DDatasetConverter:
  ...
  def read_txt(cls, txtfile):
    ...
    # for i, row in enumerate(pointcloud):
    #     print(i, " ", row)
    #     a = np.array(row).astype(np.float32)
    pointcloud = np.array(pointcloud, dtype=object).astype(np.float32)
  ...
  def convert_to_ply(cls, root_path, out_path):
    ...
    for txtfile in tqdm(txtfiles):
      ...
      for inst, subcloud in enumerate(subclouds):
        # print(subcloud)
        # with open(txtfile) as subcloud:
        #   pointcloud = [l.split() for l in subcloud]
        #   print(pointcloud)
...
def generate_splits(stanford_out_path):
  ...
  split_path = './dataset/s3dis/Spatio/splits/'
