...
class ChromaticAutoContrast(object):
  ...
  def __call__(self, coords, feats, labels):
    if random.random() < 0.2 and feats[:, :3].shape[0]>0:
  ...
  def __call__(self, coords, feats, labels):
    if random.random() < 0.95:
      for curr_ax in self.horz_axes:
          if random.random() < 0.5 and coords[:, curr_ax].shape[0]>0:
              coord_max = np.max(np.array(coords[:, curr_ax]))
...
class cfl_collate_fn_factory:
  ...
  def __call__(self, list_data):
    ...
    for batch_id, _ in enumerate(coords):
      ...
      # coords_batch.append(coords[batch_id].type(torch.int))
