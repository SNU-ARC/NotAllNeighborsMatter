...
def validate(model, val_data_loader, writer, curr_iter, config, transform_data_fn):
  v_loss, v_score, v_mAP, v_mIoU = test(model, val_data_loader, val_data_loader, config, transform_data_fn) # Put dummy val_data_loader to prevent error...
...
def train(model, data_loader, val_data_loader, config, transform_data_fn=None):
  ...
  if config.retrain_resume:
    checkpoint_fn = config.weights
    if osp.isfile(checkpoint_fn):
      state = torch.load(checkpoint_fn)
      curr_iter = state['iteration'] + 1
      epoch = state['epoch']
  ...
  while is_training:
    for iteration in range(len(data_loader) // config.iter_size):
      ...
      for sub_iter in range(config.iter_size):
        ...
        loss = criterion(soutput.features if config.backend!='torchsparse' else soutput.F, target.long())
      ...
      pred = get_prediction(data_loader.dataset, soutput.features if config.backend!='torchsparse' else soutput.F, target)
